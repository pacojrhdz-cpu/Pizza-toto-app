...
export default function ChecklistForm({ instanciaId, sucursalId, items, respuestas }: Props) {
  ...
  async function subirFoto(itemId: string, file: File) {
    ...
    const path = `${sucursalId}/${instanciaId}/${itemId}.${ext}`;
    const { error } = await supabase.storage
      .from("evidencias")
      .upload(path, file, { upsert: true });
    if (error) {
      set(itemId, { subiendo: false });
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const payload: RespuestaInput[] = items.map((it) => ({
      ...
      foto_url: estados[it.id].foto_url,
    }));
          ...
          <div key={it.id} className="rounded-xl border border-gray-200 bg-white p-4">
            ...
            <div className="mt-3">
              <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:border-toto-red">
                <span>📷 {est.foto_url ? "Cambiar foto" : "Agregar foto"}</span>
                <input
                  type="file"
                  accept="image/*"
                  capture="environment"
              ...
              {!est.preview && est.foto_url && (
                <span className="ml-2 text-xs text-green-600">Foto guardada ✓</span>
              )}
            </div>
          </div>
        );
